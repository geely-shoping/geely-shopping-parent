...
import com.geely.valid.EmployeeAccountValid;
...
public class AdminLoginController {
    ...
    @ResponseBody
    public ResponseVO AdminLogin(@ApiParam(name = "accountName,accountPassword",value = "账号名,密码",required = true) @Valid EmployeeAccountValid employeeAccountValid, BindingResult bindingResult){
        ResponseVO<Object> responseVO = new ResponseVO<>();
        if(bindingResult.hasErrors()){
            responseVO.setCode("401");
            responseVO.setMessage("账号密码不能为空!");
        }else {
            EmployeeAccountTb employeeAccountTb = employeeAccountTbService.selectByUserNameAndPassword(employeeAccountValid.getAccountName(), employeeAccountValid.getAccountPassword());
